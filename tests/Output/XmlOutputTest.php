...
final class XmlOutputTest extends TestCase
{
	...
	public function testOutput(): void
	{
		...
		$files[] = $file1 = new File(new \SplFileInfo('foo/a'), $tokens, [], new IgnoredIssues([]));
		$xmlOutput->checkedFile($file1);

		$files[] = $file2 = new File(new \SplFileInfo('foo/b'), $tokens, [], new IgnoredIssues([]));
		$file2->addIssue(new DummyCheck(), 'Error', 5);
		$xmlOutput->checkedFile($file2);

		$files[] = $file3 = new File(new \SplFileInfo('foo/c'), $tokens, [], new IgnoredIssues([]));
		$file3->addIssue(new DummyCheck(), 'Error 1', 4);
		$file3->addIssue(new DummyCheck(), 'Error 2', 12);
		$xmlOutput->checkedFile($file3);

		$files[] = $file4 = new File(new \SplFileInfo('foo/d'), $tokens, [], new IgnoredIssues([]));
		...
		$files[] = $file5 = new File(new \SplFileInfo('foo/e'), $tokens, [], new IgnoredIssues([]));
		$file5->setCheckError(new \LogicException('Serious error during file check!'));
		$xmlOutput->checkedFile($file5);

		$files[] = $file6 = new File(new \SplFileInfo('foo/f'), $tokens, [], new IgnoredIssues([]));
